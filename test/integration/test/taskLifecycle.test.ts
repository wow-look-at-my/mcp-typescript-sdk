...
describe('Task Lifecycle Integration Tests', () => {
                        ...
                        try {
                            await (shouldFail
                                ? ctx.task.store.storeTaskResult(task.taskId, 'failed', {
                                      ...
                                      errorMessage: 'Task failed as requested',
                                      isError: true
                                  })
    ...
    describe('Task Creation and Completion', () => {
        ...
        it('should handle task failure correctly', async () => {
            ...
            expect(result.errorMessage).toBe('Task failed as requested');
    ...
    describe('callToolStream with failed task', () => {
        it('should yield stored result (isError: true) when task fails, not a generic ProtocolError', async () => {
            ...
            const result = lastMessage.result as { content: Array<{ type: string; text: string }>; isError: boolean; errorMessage: string };
            expect(result.isError).toBe(true);
            expect(result.errorMessage).toBe('Task failed as requested');
            expect(result.content).toEqual([{ type: 'text', text: 'Task failed as requested' }]);
